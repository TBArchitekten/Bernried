/* Read-only integration with the existing hub. Opt in with ?minds=1. */
(() => {
  'use strict';
  if (new URLSearchParams(location.search).get('minds') !== '1') return;
  ...
  frame.title = 'MINDS//WORK · Lokaler Prototyp';
  frame.style.cssText = 'display:block;width:100%;height:calc(100dvh - 90px);min-height:640px;border:0;background:#f6f6f2';
  ...
  function publishContext() {
    ...
    try {
      ...
      add('schedule', 'Planungsterminplan LPH 3', document.getElementById('scheduleFrame').src,
        'schedule', 'Verweis auf den angezeigten Plan; Aufgaben und Fortschritt werden nicht übernommen.');
      const model = document.getElementById('modelFrame').src;
      if (model !== 'about:blank') add('model', '3D Modell', model, 'model');
      frame.contentWindow.postMessage({type:'minds:context', project:'bernried',
        projectName:document.getElementById('homeBtn').textContent, capturedAt,
        note:(usingFallback ? 'Hub-Dokumente noch nicht geladen: teilweise Repository-Fallback. ' : 'Quellen aus der Hub-Anzeige. ') +
          'Zeitstempel bezeichnet die Erfassung, nicht die Freigabe. Links können später auf geänderte Inhalte zeigen.', sources
      }, location.origin);
    } catch (error) {
      // The prototype keeps its labelled repository catalogue if the hub is unavailable.
  ...
  // Refresh snapshots when entering the view or on an explicit request from MINDS.
  // No wrapping of loadContent, DOM observers, polling or new backend queries.
